...
export const Pagination: React.FC<PropsType> = ({ idParent, nameParent, page, sizePage, portionsSize, changePage }) => {
  ...
  const _changeNewPage = (e: any) => {
    if (!e.target.value)
      return setNewPages(e.target.value);

    const value = Number(e.target.value);

    if (isNaN(value) === false && value > 0 && value <= sizePage) {
      setNewPages(value);
    }
  }
  ...
  return (
    <nav className="pagination-outer d-flex justify-content-center mt-3">
      <ul className="pagination">
        ...
        <li className="page-item">
          <input data-testid="input" value={newPages} onKeyUp={keyup} onBlur={onBlur} onChange={_changeNewPage} className='page-link' style={{ width: "60px" }} />
        </li>
